import type {
  CategoryBase,
  CollectionBase,
  CommercialModeBase,
  ProductBase,
  ProductId,
} from "../../../shared-core";

export interface ClientCategoryDto {
  id: string | number;
  key: string;
  name: string;
  parent_id?: string | number | null;
  sort_order?: number;
  is_active?: boolean;
}

export interface ClientCollectionDto {
  id: string | number;
  key: string;
  name: string;
  description?: string | null;
  status: "active" | "draft" | "archived";
  sort_order?: number;
  product_ids?: ProductId[];
}

export interface ClientCommercialModeDto {
  id: string | number;
  key: string;
  name: string;
  is_active?: boolean;
  sort_order?: number;
}

export interface ClientProductMediaDto {
  id: string | number;
  url: string;
  alt?: string | null;
  sort_order?: number;
  is_primary?: boolean;
}

export interface ClientProductVariantDto {
  id: string | number;
  sku?: string;
  name: string;
  unit: string;
  unit_key?: string | null;
  unit_name?: string | null;
  unit_kind?: "weight" | "count" | "package" | "volume" | "service" | null;
  unit_symbol?: string | null;
  unit_quantity?: string | number;
  weight_grams?: number | null;
  attributes?: Record<string, unknown>;
  is_active?: boolean;
}

export interface ClientProductPriceDto {
  id: string | number;
  variant_id?: string | number | null;
  variant_sku?: string | null;
  commercial_mode_key: string;
  commercial_mode_name: string;
  collection_key?: string | null;
  price_type: "base" | "promotional" | "subscription" | "campaign" | "manual";
  currency: string;
  amount_cents: number;
}

export interface ClientProductDto {
  id: string | number;
  key: string;
  name: string;
  slug: string;
  description?: string | null;
  unit: string;
  status: "active" | "draft" | "archived";
  is_perishable?: boolean;
  sort_order?: number;
  categories?: ClientCategoryDto[];
  primary_category_key?: string | null;
  collection_keys?: string[];
  commercial_mode_keys?: string[];
  primary_media_url?: string | null;
  media?: ClientProductMediaDto[];
  variants?: ClientProductVariantDto[];
  prices?: ClientProductPriceDto[];
}

export interface ClientCatalogQuery {
  collectionKey?: string;
  commercialModeKey?: string;
}

export interface ClientCatalogSnapshot {
  collections: CollectionBase[];
  commercialModes: CommercialModeBase[];
  products: ProductBase[];
}

export type ClientCategoryView = CategoryBase;
export type ClientCollectionView = CollectionBase;
export type ClientCommercialModeView = CommercialModeBase;
export type ClientProductView = ProductBase;
