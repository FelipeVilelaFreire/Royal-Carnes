...
const excludedSummaryKeys = new Set(["id", "image", "customerName", "name", "title", "member", "boxMonth", "statusLabel", "priceMonthlyFormatted", "totalFormatted"]);
...
export const DetailPage: React.FC<DetailPageProps> = ({ entityName, row, onBack, onEdit }) => {
  ...
  const summaryEntries = Object.entries(row).filter(
    ([k, v]) => !excludedSummaryKeys.has(k) && v !== undefined && v !== null && typeof v !== "object"
  );
    ...
    <div style={{ width: "100%", background, minHeight: "100vh", paddingBottom: "60px" }}>
      <SectionContainer atmosphere="solid" usefulColumns={20} heightRecipe="auto">
        <div style={{ display: "flex", flexDirection: "column", gap: "32px", paddingTop: "20px", width: "100%" }}>
          {/* Header de Detalhes com Avatar Executivo e Título Único */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                <AvatarCell size="lg" name={displayName} image={row.image} showName={false} />
            ...
            {activeTab === "summary" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "24px" }}>
                {summaryEntries.length === 0 ? (
                  <span style={{ color: textMuted, fontSize: "14px" }}>Nenhum atributo adicional a exibir.</span>
                ) : (
