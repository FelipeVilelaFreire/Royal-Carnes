...
function renderReadonlyValue(entry: DetailEntry, onFieldChange: DetailContentCardProps["onFieldChange"], t: AdminTranslate, viewModel: AdminStandardDetailViewModel) {
  if (entry.type === "lineItems") return <LineItemsEditor addLabel={t(entry.addLabelKey || "forms.addLineItem")} columns={(entry.columns || []) as AdminStandardLineItemColumnViewModel[]} emptyLabel={t(viewModel.emptyKey || "forms.emptyLineItems")} hierarchy={entry.hierarchy} onChange={(value) => onFieldChange?.(entry.key, value)} readOnly removeLabel={t("forms.removeLineItem")} t={t} value={Array.isArray(entry.rawValue) ? entry.rawValue : []} />;
